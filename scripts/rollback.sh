#!/usr/bin/env bash
# rollback.sh — reverts a bad deploy by moving this repo's own
# helm/employee-task/values.yaml back to its previous commit and letting
...
# `argocd app rollback` changes the live cluster without changing what's
# in Git, so the very next auto-sync would silently undo the rollback).
#
# Run this from inside a clone of employee-task-app.
#
# Usage:
#   ./scripts/rollback.sh
...
ARGOCD_APP="employee-task-dev"
VALUES_FILE="helm/employee-task/values.yaml"

echo "==> Rolling back dev (ArgoCD app: ${ARGOCD_APP})"
...
if [[ -z "${PREVIOUS_SHA}" || "${PREVIOUS_SHA}" == "${CURRENT_SHA}" ]]; then
  ...
  echo "       This may be the first deploy ever made; manual intervention required." >&2
  exit 1
fi
...
git add "${VALUES_FILE}"
git commit -m "rollback: revert to $(git rev-parse --short "${PREVIOUS_SHA}") after a failed deployment [skip ci]"
...
echo "==> Rollback complete. Run scripts/verify-deployment.sh to confirm."
