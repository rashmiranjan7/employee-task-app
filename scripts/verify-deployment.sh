#!/usr/bin/env bash
# verify-deployment.sh — smoke-tests the live deployment after ArgoCD
# reports Healthy/Synced. ArgoCD's sync status only means "the cluster
# matches Git" — it says nothing about whether the app actually serves
# traffic correctly (crashlooping pods behind a Service can still show as
# "Synced"). Run this after every deploy, or wire it into CI as a
# post-deploy gate.
#
# Usage:
#   ./verify-deployment.sh <expected-image-tag> <app-hostname>
# Example:
#   ./verify-deployment.sh dev-1bab552-66 app.yourdomain.com
#
...
EXPECTED_TAG="${1:?Usage: $0 <expected-image-tag> <app-hostname>}"
APP_HOSTNAME="${2:?Usage: $0 <expected-image-tag> <app-hostname>}"

NAMESPACE="employee-task-dev"
RELEASE="employee-task-dev"
MAX_ATTEMPTS=20
SLEEP_SECONDS=15
FAILED=0

echo "==> Verifying dev (namespace=${NAMESPACE}, expecting image tag=${EXPECTED_TAG})"
...
echo "--> Checking https://${APP_HOSTNAME}/health ..."
attempt=0
health_ok=0
while [[ ${attempt} -lt ${MAX_ATTEMPTS} ]]; do
  attempt=$((attempt + 1))
  http_code="$(curl -s -o /tmp/health-response.json -w '%{http_code}' --max-time 10 "https://${APP_HOSTNAME}/health" || echo "000")"
